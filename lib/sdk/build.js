            ...
            if (recursive && shouldProcessImport(args.path, baseURI)) {
              if (args.namespace === 'virtual') {
                if (['dynamic-import', 'import-statement'].includes(args.kind)) {
                  ...
                  // caching asset
                  globalThis[resolvedPathFetchKey] = globalThis[resolvedPathFetchKey] || fetch(pathname).then((res) => res.text())
                ...
                const textString = outputFile.text.replace(
                  ...
                  (_substring, importer) => {
                    ...
                    globalThis[resolvedPathKey] = Promise.resolve().then(async () => {
                      ...
                      // caching asset
                      globalThis[resolvedPathFetchKey] = globalThis[resolvedPathFetchKey] || fetch(resolvedPath).then(r => r.text())
