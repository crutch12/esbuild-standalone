  ...
  scripts.forEach(async (script, idx) => {
    ...
    if (script.src && processExternal || script.innerHTML && script.innerHTML.trim()) {
      ...
      const [{ config, tsconfig }, content] = await Promise.all([
        buildConfigsFetcher(),
        script.src && processExternal ? fetch(script.src).then(r => r.text()) : script.innerHTML
      ])
      ...
      const result = cachedResult || await (async () => {
        ...
        const pathname = script.src && processExternal ? new URL(script.src, document.baseURI).pathname : new URL(`./${idx}.tsx`, document.baseURI).pathname
        ...
        const { output, cacheable } = await build(
          esbuild,
          {
            [pathname]: {
              contents: content,
              loader: 'tsx', // @TODO
              src: script.src && processExternal ? script.src : undefined,
            }
          },
          {
            esbuildOptions,
            ...
            recursive,
            replaceImportMeta: recursive,
            replaceDynamicImport: recursive,
          }
        )
