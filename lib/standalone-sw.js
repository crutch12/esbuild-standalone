...
function swImportsAllowed() {
  ...
  if (/SamsungBrowser/.test(u)) return 0;
...
function getSwParams(buildParams) {
  ...
  const defaultUrl = swImportsAllowed() ? '/service-worker.mjs' : '/service-worker.js'
  ...
  const defaultType = swUrl.pathname.includes('.mjs') ? 'module' : 'classic'
  const type = searchParams.get('type') || defaultType // @NOTE: firefox doesn't properly support 'module' type
